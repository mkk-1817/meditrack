...
/**
 * Emergency Page - Enhanced with HCI Components
 * Context Menus: Emergency contacts actions (Call, Text, Email, Edit, Delete)
 * FloatingWindow: BMI calculator, Unit converter
 * Dialogs: Add/Edit contacts, Edit physician, Edit medical info
 * PopupWindow: Medical ID QR viewer
 * Toasts: All action feedb ack
 */

import { Suspense, useState, useEffect } from 'react';
import { motion } from 'framer-motion';
import { pageVariants, containerVariants, itemVariants } from '@/animations/motionVariants';
import { STORAGE_KEYS, UserProfile } from '@/lib/localStorage';
import { useLocalStorage } from '@/hooks/useLocalStorage';
import { ContextMenu } from '@/components/HCIMenus';
import { FloatingWindow, PopupWindow, MessageWindow, ToastMessage } from '@/components/HCIWindows';
...
function EmergencyContent() {
  ...
  // HCI States
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; contact: EmergencyContact } | null>(null);
  const [bmiCalculatorOpen, setBmiCalculatorOpen] = useState(false);
  ...
  const calculateBMI = () => {
    ...
    if (h > 0 && w > 0) {
      ...
      showToast('BMI Calculated', `Your BMI is ${bmi.toFixed(1)} (${category})`, 'info');
    }
  };

  const handleCall = (phone: string) => {
    window.location.href = `tel:${phone}`;
    showToast('Calling', `Dialing ${phone}...`, 'info');
  };

  const handleEmail = (email: string) => {
    window.location.href = `mailto:${email}`;
    showToast('Email', `Opening email to ${email}...`, 'info');
  };
  ...
  /* Share Medical ID function (currently unused, kept for future feature)
  const handleShareMedicalID = () => {
    ...
    if (navigator.share) {
      navigator.share({
        ...
        showToast('Shared', 'Medical ID shared successfully!', 'success');
      }).catch(() => {
        // Fallback to copy
        navigator.clipboard.writeText(shareText);
        showToast('Copied', 'Medical ID copied to clipboard!', 'info');
      });
    } else {
      navigator.clipboard.writeText(shareText);
      showToast('Copied', 'Medical ID copied to clipboard!', 'info');
  ...
  */
  ...
  const handleExportEmergencyData = () => {
    ...
    showToast('Exported', 'Emergency data exported successfully!', 'success');
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-orange-50">
      ...
      {/* Context Menu - Emergency Contacts */}
      {contextMenu && (
        <ContextMenu
          items={[
            {
              ...
              icon: "📞",
              action: () => {
                handleCall(contextMenu.contact.phone);
                setContextMenu(null);
              }
            },
            {
              ...
              icon: "💬",
              action: () => {
                window.location.href = `sms:${contextMenu.contact.phone}`;
                showToast('SMS', 'Opening messages...', 'info');
                setContextMenu(null);
              }
            },
            {
              ...
              icon: "📧",
              action: () => {
                if (contextMenu.contact.email) {
                  handleEmail(contextMenu.contact.email);
                } else {
                  showToast('No Email', 'This contact has no email address', 'warning');
                }
                setContextMenu(null);
              },
              disabled: !contextMenu.contact.email
            },
            {
              id: 'divider1',
              label: '',
            ...
            },
            {
              ...
              icon: "✏️",
              action: () => {
                showToast('Edit Contact', 'Edit dialog would open here', 'info');
                setContextMenu(null);
              }
            },
            {
              ...
              danger: true,
              action: () => {
                if (window.confirm(`Remove ${contextMenu.contact.name} from emergency contacts?`)) {
                  setEmergencyContacts(prev => prev.filter(c => c.id !== contextMenu.contact.id));
                  showToast('Contact Removed', `${contextMenu.contact.name} has been removed`, 'success');
                }
                setContextMenu(null);
              }
            }
      ...
      {/* FloatingWindow - BMI Calculator */}
      {bmiCalculatorOpen && (
        <FloatingWindow
          title="🧮 BMI Calculator"
          onClose={() => setBmiCalculatorOpen(false)}
          initialPosition={{ x: window.innerWidth / 2 - 200, y: 100 }}
          isOpen={bmiCalculatorOpen}
        >
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-body font-semibold text-navy-900 mb-2">
                Height (cm)
              </label>
              <input
                type="number"
            ...
            <motion.button
              ...
              className="w-full py-3 bg-teal-500 hover:bg-teal-600 text-white rounded-lg font-body font-medium"
            >
              Calculate BMI
            </motion.button>
            ...
            {bmiResult && (
              <motion.div
                ...
                className="mt-4 p-4 bg-gradient-to-r from-teal-50 to-blue-50 border border-teal-200 rounded-lg"
              >
                <div className="text-center">
                  <div className="text-4xl font-heading font-bold text-teal-700 mb-2">
                    {bmiResult.bmi}
                  </div>
                  <div className="text-lg font-body font-semibold text-navy-900 mb-1">
                    {bmiResult.category}
                  </div>
      ...
      {medicalIdOpen && (
        ...
        >
          <div className="space-y-6">
            <div className="text-center">
              <div className="inline-block p-4 bg-white rounded-xl border-4 border-red-500 mb-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <h4 className="text-sm font-body font-semibold text-red-900 mb-2">Personal Info</h4>
                <div className="text-sm font-body text-gray-700">
                  <p><strong>Name:</strong> {profile.personal?.firstName} {profile.personal?.lastName}</p>
                  <p><strong>Blood Type:</strong> {medicalInfo.bloodType}</p>
                  <p><strong>Organ Donor:</strong> {medicalInfo.organDonor ? 'Yes' : 'No'}</p>
                </div>
              </div>
              <div>
                <h4 className="text-sm font-body font-semibold text-red-900 mb-2">Emergency Contact</h4>
                <div className="text-sm font-body text-gray-700">
                  {emergencyContacts[0] && (
                    <>
                      <p><strong>{emergencyContacts[0].name}</strong></p>
            ...
            <div>
              <h4 className="text-sm font-body font-semibold text-red-900 mb-2">Allergies</h4>
              <div className="flex flex-wrap gap-2">
                {medicalInfo.allergies.map((allergy, idx) => (
                  <span key={idx} className="px-3 py-1 bg-red-100 text-red-700 text-sm font-body rounded-full">
                    {allergy}
                  </span>
                ))}
              </div>
            ...
            <div>
              <h4 className="text-sm font-body font-semibold text-red-900 mb-2">Current Medications</h4>
              <div className="space-y-1">
                {medicalInfo.medications.map((med, idx) => (
                  <p key={idx} className="text-sm font-body text-gray-700">• {med}</p>
                ))}
              </div>
            </div>
          </div>
      ...
      >
        ...
        <motion.section variants={itemVariants} className="mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            >
              <div className="text-4xl mb-2">🆔</div>
              <div className="text-xl font-heading font-bold mb-1">View Medical ID</div>
              <div className="text-sm opacity-90">Full Details</div>
            </motion.button>

            <motion.button
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              onClick={() => setBmiCalculatorOpen(true)}
              className="bg-gradient-to-br from-teal-600 to-teal-700 hover:from-teal-700 hover:to-teal-800 text-white p-6 rounded-xl shadow-luxury hover:shadow-luxury-hover transition-all duration-300"
            >
              <div className="text-4xl mb-2">🧮</div>
              <div className="text-xl font-heading font-bold mb-1">BMI Calculator</div>
                ...
                <div className="space-y-4">
                  {emergencyContacts.map((contact, index) => (
                    <motion.div
                      ...
                      className={`p-4 rounded-xl border transition-all duration-300 cursor-context-menu ${
                        contact.isPrimary
